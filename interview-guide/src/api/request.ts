...
// 开发：默认 baseURL 为空，请求发往当前站点（如 localhost:5173）的 /api/*，由 Vite 代理到后端（见 vite.config.ts）。
// 若需直连后端（不用代理），可在 .env.development 中设置 VITE_API_BASE_URL=http://127.0.0.1:8081
const baseURL =
