...
/** 统一换行符，避免 \\r 与动效层叠在部分浏览器里造成末行排版异常 */
    ...
    <div>
      ...
      <div className="flex flex-col gap-5">
        {suggestions.map((s: any, i: number) => {
          const issueText = normalizeMultilineText(
            typeof s === 'string' ? '问题描述' : (s.issue || '问题描述')
          );
          const recSource =
            typeof s === 'string' ? s : s.recommendation != null ? s.recommendation : '';
          const recText = normalizeMultilineText(
            typeof recSource === 'string' || typeof recSource === 'number' ? recSource : ''
          );
          return (
            <div
              key={`${priority}-${i}`}
              className={`p-5 rounded-xl border-2 ${getPriorityColor(priority)}`}
            >
              ...
              <div className="flex flex-col gap-3">
                <p className="font-semibold text-slate-900 dark:text-white text-base leading-7 break-words">
                  {issueText}
                </p>
                <div className="text-base leading-8 text-slate-700 dark:text-slate-300 whitespace-pre-line break-words">
                  {recText}
                </div>
              </div>
            </div>
