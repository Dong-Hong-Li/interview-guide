...
Vite 在 `pnpm dev` / `npm run dev` 且 `mode=development` 时会从 **`interview-guide-frontend/` 目录** 读取 `.env.development`。没有该文件时，代理目标用 **`vite.config.ts` 里的默认值**（当前为 `http://127.0.0.1:8081`）。
