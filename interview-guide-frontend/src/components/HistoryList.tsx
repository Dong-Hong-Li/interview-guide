  ...
  const loadDataSilent = useCallback(async () => {
    try {
      const [resumeData, statsData] = await Promise.all([
        historyApi.getResumes({ page: listPage, size: RESUME_LIST_PAGE_SIZE }),
        historyApi.getStatistics(),
      ]);
      setResumes(resumeData.content);
      setTotalPages(resumeData.totalPages);
      setTotalElements(resumeData.totalElements);
      setStats(statsData);
    } catch (err) {
    ...
    >
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap gap-6">
        <div>
          ...
          >
            管理您已分析过的所有简历及面试记录
            {totalElements > 0 && (
              <span className="text-slate-400"> · 共 {totalElements} 条</span>
            )}
          </motion.p>
        </div>
      ...
      {!loading && filteredResumes.length > 0 && (
        ...
        >
          ...
          {totalPages > 1 && (
            <div className="flex flex-wrap items-center justify-center gap-4 px-4 py-4 border-t border-slate-100 bg-slate-50/80">
              <button
                type="button"
                disabled={listPage <= 1}
                onClick={() => setListPage((p) => Math.max(1, p - 1))}
                className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
              >
