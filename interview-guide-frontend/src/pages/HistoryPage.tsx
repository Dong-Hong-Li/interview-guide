...
export default function HistoryList({ onSelectResume }: HistoryListProps) {
  ...
  const loadResumes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await historyApi.getResumes({ page: listPage, size: PAGE_SIZE });
    ...
    >
      ...
      <div className="flex justify-between items-start mb-10 flex-wrap gap-6">
        <div>
          ...
          >
            管理您已分析过的所有简历及面试记录
            {totalElements > 0 && (
              <span className="text-slate-400 dark:text-slate-500"> · 共 {totalElements} 条</span>
            )}
          </motion.p>
        </div>
      ...
      {!loading && filteredResumes.length > 0 && (
        ...
        >
          ...
          {totalPages > 1 && (
            <div className="flex flex-wrap items-center justify-center gap-4 px-4 py-4 border-t border-slate-100 dark:border-slate-600 bg-slate-50/80 dark:bg-slate-700/30">
              <button
                type="button"
                disabled={listPage <= 1}
                onClick={() => setListPage((p) => Math.max(1, p - 1))}
                className="inline-flex items-center gap-1 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-700 dark:text-slate-200 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
              >
