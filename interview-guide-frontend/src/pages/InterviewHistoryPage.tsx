...
// 判断是否正在评估中（展示用：含排队 PENDING）
function isEvaluating(interview: InterviewListRow): boolean {
  return interview.evaluateStatus === 'PENDING' || interview.evaluateStatus === 'PROCESSING';
}

/** 是否存在仍需轮询拉取的会话（排队/评估中，或已交卷但尚未得到正式评估结果） */
function needsInterviewListPoll(interview: InterviewListRow): boolean {
  if (interview.status === 'QUESTIONS_PENDING') return true;
  if (isInterviewEvaluateCompleted(interview)) return false;
  if (isEvaluateFailed(interview)) return false;
  if (isEvaluating(interview)) return true;
  if (isCompletedStatus(interview.status)) return true;
  return false;
}

// 判断评估是否失败
function isEvaluateFailed(interview: InterviewListRow): boolean {
  return interview.evaluateStatus === 'FAILED';
}

// 状态图标
function StatusIcon({ interview }: { interview: InterviewListRow }) {
  if (interview.status === 'QUESTIONS_FAILED') {
    return <AlertCircle className="w-4 h-4 text-red-500 dark:text-red-400" />;
  }
  if (interview.status === 'QUESTIONS_PENDING') {
    return <Loader2 className="w-4 h-4 text-amber-500 dark:text-amber-400 animate-spin" />;
  }
  // 评估失败
  ...
  // 评估完成
  if (isInterviewEvaluateCompleted(interview)) {
...
// 状态文本
function getStatusText(interview: InterviewListRow): string {
  if (interview.status === 'QUESTIONS_FAILED') {
    return '出题失败';
  }
  if (interview.status === 'QUESTIONS_PENDING') {
...
export default function InterviewHistoryPage({ onBack: _onBack, onViewInterview }: InterviewHistoryPageProps) {
  ...
  const [deleteItem, setDeleteItem] = useState<InterviewListRow | null>(null);
  ...
  const loadAllInterviews = useCallback(async (isPolling = false) => {
    ...
    try {
      const allInterviews = await fetchAllInterviewSessions();
      allInterviews.sort(
      ...
      // 计算统计信息（只统计评估已完成的面试）
      const evaluated = allInterviews.filter(i => isInterviewEvaluateCompleted(i));
  ...
  // 轮询：凡有会话处于「等待评估 / 评估中」或已交卷但尚未出分，则定时请求列表刷新状态（含 evaluateStatus、得分）
  useEffect(() => {
    const shouldPoll = interviews.some((i) => needsInterviewListPoll(i));
  ...
  const handleDeleteClick = (interview: InterviewListRow, e: React.MouseEvent) => {
    e.stopPropagation();
              ...
              <AnimatePresence>
                {filteredInterviews.map((interview, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {isInterviewEvaluateCompleted(interview) && interview.overallScore !== null ? (
                        <div className="flex items-center gap-3">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {/* 导出按钮 */}
                        {isInterviewEvaluateCompleted(interview) && (
                          <button
