...
interface ResumeDetailPageProps {
  resumeId: number;
  onBack: () => void;
  onStartInterview: (resumeText: string, resumeId: number, interviewerRole?: string) => void;
}
...
export default function ResumeDetailPage({ resumeId, onBack, onStartInterview }: ResumeDetailPageProps) {
  ...
  // 仅淡入淡出：避免整块内容带 translateX 时与子层动效叠加，在部分浏览器里出现末行文字被裁切/压扁
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        ...
        <div className="flex gap-3">
          ...
          {detailView !== 'interviewDetail' && (
            <motion.button
              onClick={() => onStartInterview(resume.resumeText, resumeId, resume.interviewerRole)}
      ...
      {/* 内容区域：不再使用 overflow-hidden，避免与动效叠层一起把建议卡片底部文字裁切 */}
            ...
            >
              {activeTab === 'analysis' ? (
                ...
                <InterviewPanel
                  ...
                  onStartInterview={() => onStartInterview(resume.resumeText, resumeId, resume.interviewerRole)}
